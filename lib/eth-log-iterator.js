  ...
  async _loop () {
    while (this.stopped === false) {
      if (asNumber(this._latestHeight) - this._confirmations <= this._height) {
        this._logger.info('Refreshing latest block height')
      ...
      this._logger.info("Fetching logs from ${this._height} to ${maxHeight} -- ${this._filters.keys().length}")
      const logsByAddress = await Promise.all(Array.from(this._filters.keys(), (addr, idx) => {
        ({ interrupt: this._blockInterrupt, promise: this._blockWait } = sleep(100 * idx))
        await this._blockWait
        return this.getLogs({
