...
module.exports = class EthLogIterator extends EventEmitter {
  ...
  async _loop () {
    while (this.stopped === false) {
      ...
      this._logger.warn(`Fetching logs from ${this._height} to ${maxHeight} -- ${this._filters.keys().length}`)
      const logsByAddress = await Promise.all(Array.from(this._filters.keys(), (addr, idx) => {
        //({ interrupt: this._blockInterrupt, promise: this._blockWait } = sleep(250 * idx))
        //await this._blockWait
        this._logger.warn(`next ${idx}`)
        return this.getLogs({
