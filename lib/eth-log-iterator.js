...
module.exports = class EthLogIterator extends EventEmitter {
  ...
  async _loop () {
    while (this.stopped === false) {
      ...
      const logs = logsByAddress
        .filter(logs => Array.isArray(logs)) // Remove non-log lists
        .flat() // Concat arrays
        .sort(sortLogs) // Sort by block, transaction index, log index
      if (logs.findIndex(l => l == null) > -1) {
        console.dir(logsByAddress)
        this._logger.error(logs)
        Array.from(this._filters.keys(), (addr, idx) => {
          this._logger.error({
            fromBlock: this._height,
            toBlock: maxHeight,
            address: addr,
  ...
  async getLogs (opts) {
    ...
    while (true) {
      ...
      try {
        ...
        if (ex.code === 'ETIMEDOUT') return // No need to log the error on timeout
