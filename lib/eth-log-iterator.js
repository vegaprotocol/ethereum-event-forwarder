...
module.exports = class EthLogIterator extends EventEmitter {
  ...
  async _loop () {
    while (this.stopped === false) {
      if (utils.parse.number(this._latestHeight) - this._confirmations <= this._height) {
        this._logger.trace('Refreshing latest block height')
        const oldHeight = this._latestHeight
        this._latestHeight = await this._eth.blockNumber('latest')
      ...
      const maxHeight = Math.min(this._height + MAX_BLOCK_RANGE, utils.parse.number(this._latestHeight) - this._confirmations)
      ...
      const logsByAddress = await Promise.all(Array.from(this._filters.keys(), addr => {
        return this._eth.getLogs({
          fromBlock: utils.format(this._height),
          toBlock: utils.format(maxHeight),
          address: addr,
          topics: [
...
function sortLogs (a, b) {
  ...
  // sort block
  // sort log index
  return utils.parse.number(b.blockNumber) - utils.parse.number(a.blockNumber) ||
    utils.parse.number(b.logIndex) - utils.parse.number(a.logIndex)
}
