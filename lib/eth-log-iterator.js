...
const MAX_BLOCK_RANGE = 25000
const BLOCK_SLEEP = 1000 * 12
...
module.exports = class EthLogIterator extends EventEmitter {
  ...
  async _loop () {
    while (this.stopped === false) {
      if (asNumber(this._latestHeight) - this._confirmations <= this._height) {
        this._logger.warn('Refreshing latest block height')
      ...
      this._logger.warn(`Fetching logs from ${this._height} to ${maxHeight} for (${this._filters.size}) filters`)
      const logsByAddress = await Promise.all(Array.from(this._filters.keys(), (addr, idx) => {
        return this.getLogs({
  ...
  async latestBlockNumber () {
    ...
    while (true) {
      ...
      try {
        ...
        if (retries % 5 === 0) this._logger.warn(`Retried eth_blockNumber('latest') ${retries} times. Latest error code: ${ex.code}`)
      }
    }
  }
  ...
  async getLogs (opts) {
    ...
    while (true) {
      retries++
      try {
        await sleep(retries * 100 * Math.random()) // Random jitter
        return await this._eth.getLogs({
          ...
          ? {
              timeout: {
                response: Math.min(retries, 45) * 1000 // Give geth more breathing room
              }
            }
          : undefined)
      } catch (ex) {
        if (ex.code === -32005) { // Returned too many events, so we split in two calls
          return this.splitGetLogs(opts)
        }
        if (retries % 5 === 0) this._logger.warn(`Retried eth_getLogs(${JSON.stringify(opts)}) ${retries} times. Latest error code: ${ex.code}`)
        if (ex.code === 'ETIMEDOUT') {
          // If we get a timeout 3 times in a row, let's release some pressure
          // and split the logs
          if (retries > 3) return this.splitGetLogs(opts)
          continue // No need to log the error on timeout
        }
  ...
  async splitGetLogs ({ fromBlock, toBlock, address, topics }) {
    const split = Math.floor((toBlock - fromBlock) / 2) + fromBlock
    this._logger.warn(`Split [${fromBlock}, ${toBlock}] into [${fromBlock}, ${split}], [${split + 1}, ${toBlock}]`)

    // Run sequentially
    const l = await this.getLogs({ address, topics, fromBlock, toBlock: split })
    await sleep(500 * Math.random()) // Random jitter
    const u = await this.getLogs({ address, topics, toBlock, fromBlock: split + 1 })

    return [l, u].then(logs => logs.flat())
  }
